...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['adicionar'])) {
        ...
        $stmt = $conn->prepare("INSERT INTO materiais (nome, capa, descricao, link, report) VALUES (?, ?, ?, ?, 0)");
        ...
        $stmt = $conn->prepare("UPDATE materiais SET nome=?, capa=?, descricao=?, link=? WHERE id=?");
        ...
        $id = $_POST['id'];
        $stmt = $conn->prepare("UPDATE materiais SET report = 0 WHERE id = ?");
...
<head>
  ...
  <title>Tensai Plus Dashboard - Gerenciamento de Materiais</title>
      ...
      <div class="sidebar">
        ...
        <nav class="mt-2">
          <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
            ...
            <li class="nav-item">
              <a href="materiais.php" class="nav-link active">
    ...
    <div class="content-wrapper">
      ...
      <div class="content-header">
        <div class="container-fluid">
          <div class="row mb-2">
            <div class="col-sm-6">
              <h1 class="m-0">Gerenciamento de Materiais</h1>
            </div><!-- /.col -->
            <div class="col-sm-6">
        ...
        <div class="container-fluid">
          ...
          <div class="card card-primary">
            <div class="card-header">
              <h3 class="card-title">Adicionar/Editar Material</h3>
            </div>
            <form method="POST">
              <div class="card-body">
                ...
                <div class="form-group">
                  ...
                  <input type="text" name="nome" id="nome" class="form-control" value="<?= htmlspecialchars($editData['nome']); ?>" required>
                </div>
                <div class="form-group">
                  <label for="capa">URL da Capa:</label>
                  <input type="text" name="capa" id="capa" class="form-control" value="<?= htmlspecialchars($editData['capa']); ?>" required>
                </div>
                <div class="form-group">
                  <label for="descricao">Descrição:</label>
                  <textarea name="descricao" id="descricao" class="form-control" rows="3" required><?= htmlspecialchars($editData['descricao']); ?></textarea>
                </div>
                <div class="form-group">
                  <label for="link">Link:</label>
                  <input type="text" name="link" id="link" class="form-control" value="<?= htmlspecialchars($editData['link']); ?>" required>
                </div>
              </div>
              <div class="card-footer">
                ...
                <?php else: ?>
                    <button type="submit" name="adicionar" class="btn btn-primary">Adicionar Material</button>
                <?php endif; ?>
              </div>
            </form>
          </div>
                ...
                <tbody>
                  <?php while ($row = $result->fetch_assoc()): ?>
                    <tr>
                      ...
                      <td>
                        <span class="badge badge-danger"><?= $row['report']; ?></span>
                      </td>
                      <td>
                        <a href="materiais.php?edit=<?= $row['id']; ?>" class="btn btn-info btn-sm"><i class="fas fa-pencil-alt"></i> Editar</a>
                        <a href="materiais.php?delete=<?= $row['id']; ?>" class="btn btn-danger btn-sm" onclick="return confirm('Tem certeza que deseja excluir este material?');"><i class="fas fa-trash"></i> Excluir</a>
